...
function generateQuestions(region: Region, count: number): Question[] {
  const words = shuffle(region.words).slice(0, count);
  return words.map((word) => {
    ...
    if (typeRoll < 0.4) {
      ...
      const others = shuffle(
        region.words.filter((w) => w.chinese !== word.chinese)
      )
        .slice(0, 3)
        .map((w) => w.chinese);
      ...
      const others = shuffle(
        region.words.filter((w) => w.japanese !== word.japanese)
      )
        .slice(0, 3)
        .map((w) => w.japanese);
      questionOptions = shuffle([correctAnswer, ...others]);
...
export default function Quiz({ region, onBack, onComplete }: QuizProps) {
  const questionCount = Math.min(10, region.words.length);
  const questions = useMemo(
    () => generateQuestions(region, questionCount),
  ...
  const handleNext = useCallback(() => {
    if (currentIndex + 1 >= questions.length) {
      setFinished(true);
      onComplete(
        correct + (selected === question.correctIndex ? 0 : 0),
        questions.length
      );
    } else {
  ...
  }, [currentIndex, questions.length, correct, selected, question, onComplete]);
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 text-center max-w-sm w-full">
          ...
          <div className="text-4xl font-bold my-4" style={{ color: region.color }}>
            {score}/{total}
          </div>
