  ...
  return (
    <div className="grid gap-3">
      {regions.map((region) => {
        ...
        // Aggregate progress across all prefectures in this region
        const learnedCount = region.prefectures.reduce((s, p) => {
          return s + getRegionProgress(p.id, p.words.length).count;
        }, 0);
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400">
                {region.prefectures.length} 個地區 · {totalWords} 個單字
              </div>
              ...
              <div className="text-xs text-gray-400 mt-0.5">
                {learnedCount}/{totalWords} 已學會
              </div>
            </div>
            <svg className="w-5 h-5 text-gray-300 dark:text-gray-600 shrink-0 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
